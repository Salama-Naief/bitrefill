   ...
   <Layout title="login">
      ...
      <div className={` container mx-auto flex justify-center  px-4 font-sans`}  style={{direction:i18n.language==="ar"?"rtl":"ltr"}}>
         <div className={` bottom-0 right-0 w-full md:w-1/2 px-4 md:px-8`}>
               ...
               <div className='my-2'>
                    <input type="checkbox" onChange={(e)=>setCodeCheckbox(e.target.checked)} className={`checked:bg-black my-3`}/><span className='mx-2  text-sm text-gray-500'>I have a referral code</span>
                    ...
                    <input type="checkbox" onChange={(e)=>setNewsletterCheckbox(e.target.checked)} className={`checked:bg-black my-3`}/><span className='mx-2  text-sm text-gray-500'>Add me to the newsletter to receive news about new products and features</span><br/>
                    <input type="checkbox" onChange={(e)=>setAgreeCheckbox(e.target.checked)} className={` indeterminate:bg-slate-500 checked:bg-black my-3`}/><span className='mx-2  indeterminate:bg-secondary-200 text-sm text-gray-500'>I have read and agree with the Bitrefill <Link href={"/"}><a className=" underline font-semibold">Terms & Conditions</a></Link>  and the <Link href={"/"}><a className=" font-semibold underline">Privacy Policy</a></Link> </span>
                 </div>
